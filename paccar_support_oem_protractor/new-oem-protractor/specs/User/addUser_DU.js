 ...
 */
describe("Validating add a Dealer User -----> ", function () {
    ...
    var userRole = browser.params.roleslabels.dealeruser;
    ...
    loginUserArray.forEach((eachUser) => {
        it("As a" + eachUser + ", I complete the add user form for a verified test user Dealer User", () => {
            loginPage.get()
                .then(() => {
        ...
        it("Check to see that the new test user has all the correct navigation links for a Dealer User", () => {
            loginPage.get();
            loginPage.login('paccar', testUserEmail, password);
            userUtility.verifyLinkVisibility(userRole);
        });

        it("The Test User can add users of all other user types visible to a Dealer User", () => {
            loginPage.get();
            loginPage.login('paccar', testUserEmail, password);
